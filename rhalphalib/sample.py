...
>>>>>>> 79aa39ada2560c5c705200489ae8e8b9faf64d46
            ...
            param = NuisanceParameter(name + '_mcstat', combinePrior='shape')
            self.setParamEffect(param, effect_up, effect_down)
        else:
            for i in range(self.observable.nbins):
                if self._nominal[i] <= 0.:
                    continue
                if self._sumw2[i] <= 0.:
                    continue
                if np.sqrt(self._sumw2[i]) <= 5e-3:
                    continue
                ### This is a test, we will not create nuisances for those bins that have high statistics 
                ### The effect of sqrt(sumw[i]) (that is the uncertainty) on the nominal (that is the number of events). 
                ### If the effect is smaller than a certain threshold (we start from 1%), we don't create the nuisance
                if np.sqrt(self._sumw2[i])/self._nominal[i] <= 5e-3:
                    continue
                effect_up = np.ones_like(self._nominal)
                effect_down = np.ones_like(self._nominal)
                ...
                self.setParamEffect(param, effect_up, effect_down)
                #print("Print autoMCStats")
                #print("Name, bin, nominal, sumw2, sqrt(sumw2), effect up, effect down")
                #print(self.name, i, self._nominal[i], self._sumw2[i], np.sqrt(self._sumw2[i]), effect_up[i], effect_down[i], "\n")
